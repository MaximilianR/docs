...
// Import images using the Docusaurus alias for the static directory
...
export default function ScrvusdVaultTicker() {
  ...
  useEffect(() => {
    ...
    // --- 2. Dynamic Price Ticker Logic ---
    let priceInterval; // To hold the interval ID for cleanup
    ...
    const fetchAndUpdatePrice = async () => {
      try {
        ...
        const [initialPriceBigInt, decimals, savingsResponse] = await Promise.all([
          contract.pricePerShare(),
          contract.decimals(),
          fetch(savingsApiUrl)
        ]);

        if (!savingsResponse.ok) throw new Error('Failed to fetch savings API data.');
    ...
    return () => {
      ...
      anime.remove('*'); // Stop all anime.js animations
    };
  }, []); // The empty array [] means this effect runs only once after the initial render

  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <svg width="1400" height="500" viewBox="0 0 1400 500" fill="none" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
            ...
            <defs>
                ...
                {/* Repeating patterns for the logo line */}
                {[...Array(5).keys()].map(i => (
                    <pattern key={i} id={`pattern_crvusd_logo_line_${i}`} patternContentUnits="objectBoundingBox" width="1" height="1">
                        <use xlinkHref="#img_crvusd_logo" transform="scale(0.02)"/>
                    </pattern>
                ))}
                {/* Image definitions using imported paths */}
                <image id="img_scrvusd_logo_big" width="120" height="120" preserveAspectRatio="none" href={ScrvusdLogoBig}/>
                <image id="img_scrvusd_logo_small" width="50" height="50" preserveAspectRatio="none" href={ScrvusdLogoSmall}/>
                <image id="img_crvusd_logo" width="50" height="50" preserveAspectRatio="none" href={CrvusdLogo}/>
            </defs>
        </svg>
    </div>
  );

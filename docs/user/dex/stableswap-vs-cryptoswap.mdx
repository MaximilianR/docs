...
A key feature of Stableswap pools is that they are **fully passive**. Unlike other AMMs like Uniswap v3 where LPs must actively manage their price ranges, Curve LPs can simply deposit their assets and earn fees without further intervention. The pool provides liquidity across all possible prices, though it's most concentrated at the peg.
...
<figure style={{ textAlign: 'center' }}>
  <ThemedImage
    ...
    sources={{
      light: require('@site/static/img/user/dex/stableswap-liquidity.png').default,
      dark: require('@site/static/img/user/dex/stableswap-liquidity.png').default,
    }}
    style={{
      maxWidth: '800px',
      width: '100%'
...
<figure style={{ textAlign: 'center' }}>
  <ThemedImage
    ...
    sources={{
      light: require('@site/static/img/user/dex/stableswap-blocks.png').default,
      dark: require('@site/static/img/user/dex/stableswap-blocks.png').default,
    }}
    style={{
      maxWidth: '1400px',
      width: '100%'
...
2.  The swap executes with extremely low slippage, and the user receives nearly \$19.98M crvUSD.
...
<figure style={{ textAlign: 'center' }}>
  <ThemedImage
    ...
    sources={{
      light: require('@site/static/img/user/dex/cryptoswap-liquidity.png').default,
      dark: require('@site/static/img/user/dex/cryptoswap-liquidity.png').default,
    }}
    style={{
      maxWidth: '800px',
      width: '100%'
...
2.  **Profitability Check**: The pool must have accrued enough trading fees to ensure the rebalancing cost is less than half of the generated profits. This guarantees the rebalancing action itself doesn't cause a net loss for LPs.
...
<figure style={{ textAlign: 'center' }}>
  <ThemedImage
    ...
    sources={{
      light: require('@site/static/img/user/dex/cryptoswap-rebalance.png').default,
      dark: require('@site/static/img/user/dex/cryptoswap-rebalance.png').default,
    }}
    style={{
      maxWidth: '1200px',
      width: '100%'
...
In this example the pool was last balanced around the price of \$1.17, and the pool has a minimum step of \$0.01, the current price is \$1.175.  Because the Euro goes up in value to \$1.18, this is equal to the price movement threshold, and the pool rebalances around this new price, assuming it generated enough profits.

1.  **Initial State**: The pool's current price is \$1.175. It was last balanced around the price of \$1.17, and it has a minimum step of \$0.01.
2.  **Price Movement**: The market price of the Euro strengthens to \$1.18. The internal oracle in the Curve pool detects this change.
3.  **Rebalancing Trigger**: The new price is equal or greater than the minimum step, and in this example, we assume the pool has accumulated sufficient fees.
4.  **Automatic Rebalancing**: The algorithm uses a portion of the accrued fees to rebalance the pool's liquidity, centering it around the new \$1.18 price.
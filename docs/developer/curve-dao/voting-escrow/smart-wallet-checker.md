...
The SmartWalletChecker is an external contract that checks if certain smart contracts are approved to lock CRV tokens into the VotingEscrow. **Permission can be granted via the `approveWallet` and revoked via `revokeWallet` methods.**:::deploy[Contract Source & Deployment]
...
<details>
<summary>VotingEscrow: Internal `assert_not_contract` function</summary>
...
## **Approve/Revoke SmartContracts**### `approveWallet`

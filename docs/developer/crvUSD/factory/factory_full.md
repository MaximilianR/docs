...
## **Adding Markets and Adjusting Debt Ceilings**### `add_market`
...
**There are two possibilities on how to set the debt ceiling:**1. When raising the debt ceiling, the difference between `debt_ceiling` and `debt_ceiling_residual` will be minted to the controller. 
...
## **Implementations**The implementations are based on blueprint contracts specified in [EIP-5202](https://eips.ethereum.org/EIPS/eip-5202). When a new market is added, an AMM (Automated Market Maker) and a Controller contract are created from the corresponding blueprints. New implementations can be set by the `admin` of the factory.
